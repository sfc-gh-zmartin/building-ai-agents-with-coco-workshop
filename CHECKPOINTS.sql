...
WHERE CREATED_AT >= DATEADD('day', -30, (SELECT MAX(CREATED_AT) FROM GITTREND_DB.PUBLIC.GITHUB_EVENTS))
GROUP BY EVENT_TYPE
...
WHERE EVENT_TYPE = 'WatchEvent'
  AND CREATED_AT >= DATEADD('day', -30, (SELECT MAX(CREATED_AT) FROM GITTREND_DB.PUBLIC.GITHUB_EVENTS))
LIMIT 20;
